...
from src.platforms.instagram.replies import fetch_replies
...
async def run(config: dict) -> dict:
    ...
    async with async_playwright() as p:
        ...
        if posts:
            ...
            # Save the scrape immediately — media + comments are best-effort
            ...
            # Comments — bounded; a wedged post tab can't stall the run.
            ...
            if reply_posts:
                post_dicts = [{"id": p.id, "url": p.url} for p in reply_posts]
                try:
                    ...
                    warnings.append(f"reply phase exceeded {REPLY_PHASE_TIMEOUT}s; saved without comments")
                except Exception as e:
                    warnings.append(f"reply phase error: {e}")

            # Re-save with media paths + comments attached.
            save_posts(unique_posts, run_dir, platform="instagram", duration_seconds=duration)
        else:
            unique_posts = []
            downloaded = 0
